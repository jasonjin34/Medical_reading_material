...
**图1.** HEST 总览:1,108 对空间转录组(ST)与配对 H&E 全切片图像组成 HEST-1k;HEST-Library 提供数据统一化功能;下游应用包括基础模型基准测试、生物标志物发现与多模态表征学习。
...
**图2.** 生物标志物发现:在浸润性导管癌(IDC)Xenium 样本上,展示 neoplastic 核叠加、GATA3 表达热图、核面积分布、CellViT 分割示例,以及形态学特征与基因表达的相关性(如核面积 ↔ GATA3)。
_Source: https://arxiv.org/html/2406.16192v1/x2.png  ·  License: arXiv (open) / paper CC BY-NC-SA 4.0_

### 结果表

**表1.** HEST-Benchmark:10 个基因表达预测任务上 10 个病理基础模型的 Pearson 相关(均值±标准差,patient-stratified k-fold 交叉验证)。**加粗** = 该任务最优。平均列(Average)按 mean Pearson 排序,UNI 最高(0.319),CONCH 单任务(IDC/LYMPH_IDC)领先。
...
_Source: https://arxiv.org/html/2406.16192v1 (Table 1) · 标准差已省略以便阅读;完整 ±sd 见原文。_
